...
class TestVendorDashboard:
    ...
    def test_dashboard_redirects_unauthenticated_user(self, client):
        response = client.get(reverse('vendor:dashboard'))
    ...
    def test_dashboard_uses_correct_template(self, client, user):
        client.force_login(user)
        response = client.get(reverse('vendor:dashboard'))
        assert 'vendor/dashboard.html' in [t.name for t in response.templates]
    ...
    def test_dashboard_contains_vendor_role_badge(self, client, user):
        client.force_login(user)
        response = client.get(reverse('vendor:dashboard'))
